...
Jarl is available as a command-line tool and as an extension for Positron, VS Code, Zed, and more.
...
* **standard comments** apply to the next block of code, or *node*.
  A node correspond to an R expression as well as all expressions that belong to it (aka *children nodes*).
  Importantly, the detection of which node is affected by a comment doesn't depend on the layout of the code (line breaks, whitespaces, etc.).
  ...
  If you have ever used [Air's suppression comments](https://posit-dev.github.io/air/formatter.html#disabling-formatting), then you should already be familiar about the locations of Jarl's comments since they follow the same rules.

  <details>
  <summary>Click to see more details on how node detection works.</summary>
  ...
  The top node is the assignment `<-`.
...
For instance, suppose I have `test.R` in my working directory, and this working directory doesn't have a `jarl.toml`.
Before 0.4.0, Jarl wouldn't look for other configuration files.
As of 0.4.0, Jarl looks in parent folders and would find `jarl.toml` one level above.
...
│   └── test.R
    |
└── jarl.toml  # ---> this is in the parent directory (relative to the wd)
```
...
If you find any issues or want to contribute, please check out the [Github repository](https://github.com/etiennebacher/jarl). And thanks Maëlle for the suggestions!
