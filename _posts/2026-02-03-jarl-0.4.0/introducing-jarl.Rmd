...
* **standard comments** apply to the next block of code, or *node*.
  ...
  In the example above, `any(is.na(x1))` and `any(is.na(x2))` are two separate nodes.
  The assignment operation (`<-`) is also a node that has two children: the left-hand side of the assignment (`f`) and the right-hand side of the assignment (the function definition).
  Therefore, the two `any(is.na(...))` in the function body are two separate nodes, but they belong to the same parent node.
  The parent node has Jarl's special comment, hence none of the two `any(is.na(...))` is reported.
