...
#show: template.with(
  ...
  date: datetime(year: 2021, month: 12, day: 23),
  lang: "en",
)

= Reproduce some maps about 3G and 4G access
<reproduce-some-maps-about-3g-and-4g-access>
...
Now, we could plot all those points, but there are so many that it would completely hide the map so we need to use the smallest size possible for each point. Contrary to what one might expect, this is not done with the `size` argument but with the `shape` argument. Indeed, setting `shape = "."` means that each point will be the size of one pixel#footnote[Thanks to #link("https://stackoverflow.com/a/51700192/11598948")[this StackOverflow answer] for pointing that out.]. Still there are too many points compared to the original plot. The solution here is to decrease the opacity of each point:
