...
- Jarl has a new experimental rule (`unused_object`) to detect objects that are defined but never used. This is disabled by default because there may be false positives. However, I released it both because it can already be very useful and because it can be improved thanks to your bug reports!
