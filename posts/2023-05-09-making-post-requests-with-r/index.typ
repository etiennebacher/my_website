...
#set document(title: [Making POST requests with R])
#metadata((
  title: "Making POST requests with R",
  kind: "post",
  date: "2023-05-09",
  summary: "An alternative to RSelenium to get data from dynamic webpages.",
)) <website-metadata>
...
<making-post-requests-from-r>
There are two main packages to do this: `httr` and `httr2`. The latter is just a couple of years old and has being developed with a lot of convenient built-in features (retries if a query failed, secure secrets, etc.#calepin.elements.sidenote[I'm just listing things that are written on the #link("https://httr2.r-lib.org/")[website of `httr2`], more details there.]).
