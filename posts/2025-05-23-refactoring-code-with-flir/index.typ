...
#set document(title: [Refactoring code with flir])
#metadata((
  title: "Refactoring code with flir",
  kind: "post",
  date: "2025-05-23",
  summary: "How to efficiently rewrite many code patterns.",
)) <website-metadata>
...
== Creating the rule
<creating-the-rule>
Now we must define the patterns we want to detect in the code. For now, let's say we want to catch usages of `sample_n(my_data, 10)` and `sample_n(my_data, size = 10)` and replace them with `slice_sample(my_data, n = 10)`#calepin.elements.sidenote[`my_data` and `10` are just placeholders here, they could be anything.]. We want to detect two patterns and replace them with the same code, so we can use `any` in this situation:
