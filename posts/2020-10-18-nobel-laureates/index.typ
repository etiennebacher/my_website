...
#set document(title: [Visualize data on Nobel laureates per country])
#metadata((
...
<import-and-clean-data>
Now, we can import the dataset. To remove the capital letters and transform the column names in snake case (i.e names such as "column\_name" instead of "Column Name"), we can use the function `clean_names()` of the package `{janitor}` #calepin.elements.sidenote[This function is very useful even when column names are much more messy.]:
